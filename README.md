...
- `/info/unit-config.json` - Edit this to change which mission loads initially, set the pilot information, and other ease-of-use options that have been introduced to modify how the site works. Below are the values you might have to change.
...
## Templating this site for other units

Most unit-specific info lives in **`src/config/unit-config.json`**:
- Unit name + header labels
- Planet video path
- Default title + favicon
- Google Sheets CSV URLs (members / ref data / ops)
- Deployment Netlify function URL

To clone for a new unit:
1. Copy this repo.
2. Replace assets in `/public` (logos, planet video, ranks if needed).
3. Edit **`src/config/unit-config.json`** only.
4. (Optional) override at deploy-time via Vite env vars:
   - `VITE_MEMBERS_CSV_URL`
   - `VITE_REF_DATA_CSV_URL`
   - `VITE_OPS_CSV_URL`
   - `VITE_DEPLOYMENT_EXEC_URL`

Legacy compatibility: `index.html` can still define `window.*` globals, but you shouldn't need it for new templates.
