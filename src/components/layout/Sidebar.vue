...
<template>
  <div class="header-wrap" :style="{ '--auth-x': authOffsetX + 'px', '--auth-y': authOffsetY + 'px' }">
    <header>
      ...
      <!-- Title (fixed text, logo locked to 120x120) -->
 ...
 * 1) runtimeConfig.branding.headerLogo
 * 2) runtimeConfig.branding.logo
 * 3) runtimeConfig.icon   (from unit-config.json)
 * 4) fallback: /faction-logos/UNSC_CENTCOM_LOGO.png
 *
...
/* Logo locked square and fills header nicely */
.logo {
  width: 120px;
  height: 120px;
  object-fit: contain;
...
/* Auth indicator pill (RIGHT aligned, not absolute) */
.auth-indicator {
  position: absolute;
  right: 14px;
  top: 10px;
