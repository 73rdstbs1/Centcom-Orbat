...
export default {
  ...
  mounted() {
    this.scheduleLogoPreload();
    this.recalcTickerDuration();
  },
  beforeUnmount() {
    this.clearLogoPreloadTimers();
  ...
  methods: {
    preloadUnitLogos() {
      ...
      srcs.forEach((s) => {
        ...
        // Force a fetch even if the header is hidden behind the gate.
        const img = new Image();
        img.decoding = "async";
        img.src = url;
      });
    ...
    clearLogoPreloadTimers() {
      const timers = Array.isArray(this._logoPreloadTimers) ? this._logoPreloadTimers : [];
      timers.forEach((t) => {
        try {
          clearTimeout(t);
        } catch {}
      });
      this._logoPreloadTimers = [];
    },
    ...
    scheduleLogoPreload() {
      ...
      // Retry a couple times to catch late runtime-config hydration or slow networks.
      const delays = [250, 1200, 2800];
      this._logoPreloadTimers = delays.map((ms) =>
        setTimeout(() => {
          this.preloadUnitLogos();
          this.fixBrokenLogos();
        }, ms),
    ...
    fixBrokenLogos() {
      ...
      const imgs = Array.from(root.querySelectorAll("img.unit-logo"));
      imgs.forEach((img) => {
        if (!img) return;
        ...
        if (!src) return;
        this.retryLogoSrc(img, src);
      });
    },

    retryLogoSrc(imgEl, src) {
      const key = String(src || "").trim();
      if (!key) return;

      const seen = this._logoRetryCount || {};
      const tries = Number(seen[key] || 0);
      if (tries >= 2) return;

      this._logoRetryCount = { ...seen, [key]: tries + 1 };

      const bust = `v=${Date.now()}_${tries + 1}`;
      const next = key.includes("?") ? `${key}&${bust}` : `${key}?${bust}`;
    ...
    onLogoError(e, src) {
      const imgEl = e && e.target ? e.target : null;
      if (!imgEl) return;
      this.retryLogoSrc(imgEl, src);
    },
